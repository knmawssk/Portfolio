...
# 1. Review Distribution (Sentiment Pie Chart)
...
generate_wordcloud(
    df[df['sentiment'] == 1]['Описание'],
    'Frequent Words in Positive Reviews'
)
generate_wordcloud(
    df[df['sentiment'] == 0]['Описание'],
    'Frequent Words in Negative Reviews'
)

# 3. Confusion Matrix (if you have true labels)
# Assuming you have a 'true_sentiment' column with ground truth
if 'true_sentiment' in df.columns:
    cm = confusion_matrix(df['true_sentiment'], df['sentiment'])
